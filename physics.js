...
class Physics
{
    ...
    for (let obj of this.movingObjects)
    {
      ...
      for (let collidableObj of buckets)
      {
        ...
        if (willCollide)
        {
          ...
          {
            this.movingObjects.splice(this.movingObjects.indexOf(obj));
            tempMovingObjectsContainer.push(obj);

            this.movingObjects.splice(this.movingObjects.indexOf(collidableObj));
  ...
  {
    let object_hitbox = object.hitbox;
    // if(object.prevX == undefined || object.prevY == undefined)
    // return;

    let i = floor(object.prevX-object_hitbox.width/2 / bucketMap_one_cell_width);
    let j = floor(object.prevY - object_hitbox.height / bucketMap_one_cell_height);
    i = clamp(i,0,bucketMap_how_many_horizontal_cell);
    j = clamp(j,0,bucketMap_how_many_vertical_cell);
    ...
    i = floor(object.prevX-object_hitbox.width/2 / bucketMap_one_cell_width);
    j = floor(object.prevY / bucketMap_one_cell_height);
    i = clamp(i,0,bucketMap_how_many_horizontal_cell);
    j = clamp(j,0,bucketMap_how_many_vertical_cell);
    ...
    i = floor(object.prevX+object_hitbox.width/2 / bucketMap_one_cell_width);
    j = floor(object.prevY - object_hitbox.height / bucketMap_one_cell_height);
    i = clamp(i,0,bucketMap_how_many_horizontal_cell);
    j = clamp(j,0,bucketMap_how_many_vertical_cell);
